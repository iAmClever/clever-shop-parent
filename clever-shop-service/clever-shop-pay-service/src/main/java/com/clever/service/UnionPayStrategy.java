...
import com.clever.sdk.SDKConfig;
import com.clever.sdk.UnionPayBase;
...
public class UnionPayStrategy{
	...
	public String toPayHtml(PaymentChannelEntity paymentChannel, PayMentTransacVO payMentTransacVO) {
		log.info(">>>>>>>>银联支付组装参数开始<<<<<<<<<<<<");
		...
		String paymentId = payMentTransacVO.getPaymentId();
		requestData.put("orderId", paymentId); // 商户订单号，8-40位数字字母，不能含“-”或“_”，可以自行定制规则
		requestData.put("txnTime", format(payMentTransacVO.getCreatedTime())); // 订单发送时间，取系统时间，格式为YYYYMMDDhhmmss，必须取当前时间，否则会报txnTime无效
		requestData.put("currencyCode", "156"); // 交易币种（境内商户一般是156 人民币）
		Long payAmount = payMentTransacVO.getPayAmount();
